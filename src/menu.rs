...
use basalt::input::{Character, InputHook, InputHookData, InputHookRes, MouseButton};
use basalt::interface::bin::{Bin, BinPosition, BinStyle, Color};
		...
		self.entry_bin.on_mouse_press(
			...
			Arc::new(move |_, _| {
				let exec = entry
					...
					.replace("%u", "")
					.replace("%U", "")
					.replace("%i", entry.icon.clone().as_ref().map(|v| v.as_str()).unwrap_or(""))
...
impl Menu {
	pub fn new(basalt: Arc<Basalt>, categories: Vec<Category>, entries: Vec<Arc<DesktopEntry>>) -> Arc<Self> {
	...
	fn display_search(&self, text: String) {
		...
		entries.sort_by_key(|e| {
			(strsim::jaro_winkler(e.name.as_str(), text.as_str()) * u64::max_value() as f64).floor() as u64
		});
